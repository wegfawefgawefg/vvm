...
void test_affinity_retain_raises_good_op_affinity() {
    ...
    train_config.learning_rate = 0.5F;
    train_config.max_grad_norm = 10.0F;
    train_config.affinity_retain_scale = 1.0F;
    train_config.affinity_retain_threshold = 1.0F;
    (void)model.train_window(std::span<const vvm::Tick>(&tick, 1), train_config);
...
void test_affinity_retain_underuse_gate_blocks_overused_op() {
    vvm::Config config{};
    config.state_dim = 8;
    config.num_ops = 16;
    config.candidate_count = 4;
    ...
    train_config.affinity_retain_scale = 1.0F;
    train_config.affinity_retain_threshold = 1.0F;
    train_config.affinity_retain_underuse_scale = 1.0F;
    ...
    assert(std::fabs(after_affinity - before_affinity) < 1.0e-6F);
}
